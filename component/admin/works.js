...
class Admin_works extends Component{
...
delete= id => () =>{
  ...
  firebase.database().ref().child('works').orderByChild('id').equalTo(id).once('value').then( snapshot => {
    let allNotes = [];
    let key_temp=[];
    snapshot.forEach(snap => {
      allNotes.push(snap.val());
    ...
    firebase.database().ref('works/'+key_temp).remove() 
...
// alert(fire.database().ref().child('projects'));  
    firebase.database().ref().child('works').push(
{
  // name:name,
  ...
  // this.setState({img_url:file.name});
    ...
    let task=fileRef.put(file);
     ...
     () => {
      ...
      firebase.storage().ref('works').child(str_name).getDownloadURL()
       .then(fireBaseUrl => {
         // alert(fireBaseUrl);
      ...
      <div class="navbar-inner">
        ...
        <div class="collapse navbar-collapse" id="sidenav-collapse-main">
          ...
          <ul class="navbar-nav">
            ...
            <li class="nav-item">
              <Link className="nav-link active" to={"/admin/Works"}>
  ...
  <div class="main-content" id="panel">
    ...
    <div class="container-fluid mt--6 ">
      <div class="row">
        ...
        <div class="content">
            <h5 style={{color:"#fff"}} >{note.name}</h5>
            <h5 style={{color:"#fff"}} >{note.type}</h5>
            
        </div>

    </div>
    <a onClick={this.delete(note.id)} style={{color:"blue",cursor:"pointer"}}>Delete</a>
    </div>
            ...
            <div class="card-header border-0">
              <div class="row align-items-center">
                ...
                <div style={{marginBottom: "1.5rem"}}>
                      <label class="ip_work_label">Name</label>
                      <input type="text" id="name"  class="ip_work"/>
                      </div>
                    </div>
                    <div class="col-lg-6">
                ...
                <div class="col-lg-6 text-left">
                <div style={{marginBottom: "1.5rem"}}>
                      <label class="ip_work_label">Link</label>
                      <input type="text" id="link"  class="ip_work"/>
                      </div>
                    </div>
                ...
                <div class="col-md-4">
                  <label for="files" class="btn car_cv_file_label1">Add Image</label>
                      ...
                      <p style={{color:"red", fontSize:"12px"}}>Image Size:500px Width, 370px Height</p>
                </div>
                <div class="col-md-2">
